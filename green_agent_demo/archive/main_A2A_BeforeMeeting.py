...
class EcomGreenAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        """Main execution flow - handles both baseline and white agent modes"""
        ...
        try:
            ...
            if use_baseline or not white_agent_url:
                ...
                mode = "white_agent"
                print(f"Green agent: Mode=WHITE_AGENT for user {user_id} (agent at {white_agent_url})")
    ...
    def _parse_task_config(self, user_input: str) -> Dict[str, Any]:
        """
        Parse incoming task configuration from launcher
        Expected format: JSON with white_agent_url, user_id, etc.
        """
        try:
            # If using XML tags (like tau-bench example)
            if "<white_agent_url>" in user_input:
                from ab_src.my_util import parse_tags
                tags = parse_tags(user_input)
                config = json.loads(tags.get("task_config", "{}"))
                config["white_agent_url"] = tags["white_agent_url"]
                return config
            else:
                # Direct JSON
                return json.loads(user_input)
        except Exception as e:
            raise ValueError(f"Failed to parse task config: {e}")
    ...
    async def _white_agent_policy(
        ...
        White agent should use the tools to make predictions
        """
            ...
            "actions": [
                {
                    ...
                    "method": "GET",
                    "path": "/search",
                    "description": "Search for products by name, aisle, or department",
                    "params": {
                        "q": "search query",
                        "field": "name|aisle|department",
                ...
                },
                {
                    ...
                    "method": "POST",
                    "path": "/checkout",
                    "description": "Finalize the cart",
                    "body": {"agent_key": agent_key}
                }
            ]
        }
        
        # Build task message for white agent
        task_message = f"""
            {prompt}
            ...
            2. Use cart_add to build your basket with your session key
            3. Use checkout with your session key when done
            4. Return JSON with your final prediction:
        ...
        await event_queue.enqueue_event(
            new_agent_text_message("Sending task to white agent...")
        )
    ...
    def _build_task_message(self, prompt: str, tools: Dict, user_id: int) -> str:
        """Format task message for white agent"""
        return f"""
            {prompt}
            ...
            Use the provided tools to search products and build the basket.
            """
